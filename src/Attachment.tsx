...
const   Media = (props: {
    ...
    switch (props.type) {
        ...
        default:
            return <img { ... props } className="wc-card-image"/>;
    }
}
...
const title = (title: string) => renderIfNonempty(title, title => <h1 className="wc-card-title">{ title }</h1>);
...
export const AttachmentView = (props: {
    ...
    const buttons = (buttons: CardAction[]) => buttons &&
        <ul className="wc-card-buttons">
            { buttons.map((button, index) => <li key={ index }><button className="wc-card-buttons-individual" onClick={ onCardAction(button) }>{ button.title }</button></li>) }
        </ul>;

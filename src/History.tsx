                ...
                <div className="wc-message-group-content" ref={ div => this.scrollContent = div }>
                    { this.props.activities.map((activity, index) =>
                        <WrappedActivity
                            ...
                            hideTimestamp={ index < this.props.activities.length-1 && index > 1 && this.props.activities[index+1].from.id===this.props.activities[index].from.id}
                            selected={ activity === this.props.selectedActivity }
                            fromMe={ activity.from.id === this.props.user.id }
                            format={ this.props.format }
                            prevWho={this.props.prevWho}
                           ...
                           hideUserIcon={index > 0 && this.props.activities[index-1].from.id===this.props.activities[index].from.id}
                        />
                    ) }
                </div>
            </div>
...
export class WrappedActivity extends React.Component<WrappedActivityProps, {}> {
    ...
    render () {
        ...
        //sthis.props.prevWho = this.props.prevWho ? who : this.props.prevWho;

        const prevWho = this.props.prevWho;
...
/*
        ...
        return (
            ...
            <div data-activity-id={ this.props.activity.id } className={ wrapperClassName } onClick={ this.props.onClickActivity }>
                <div className={ 'myIcon-'+who+((this.props.hideUserIcon || prevWho === who)?"-groupconv":"")}></div>
                <div className={ 'wc-message wc-message-from-' + who } ref={ div => this.messageDiv = div }>
                    <svg className={'wc-message-callout wc-message-callout-'+who}>
                        <path className="point-right" d="M0 0 L3 3 L0 6 Z" />
                        <path className="point-left" d="M0 3 L3 0 L3 6 Z" />
                    </svg>
                ...
                <div className={ 'wc-message-from wc-message-from-' + who  + (this.props.hideTimestamp ? '-hide':'') }>{ timeLine }</div>
            </div>
        );
    }
}